...
This codemod transforms @kazuki.m---strapi/utils imports to change method calls to match the new public interface.
...
import { nameToSlug } from '@kazuki.m---strapi/utils';
...
const { strings } = require('@kazuki.m---strapi/utils');
...
const { nameToSlug } = require('@kazuki.m---strapi/utils');
...
const transformImports = (root: Collection, j: JSCodeshift) => {
  root
    .find(j.ImportDeclaration, {
      source: { value: '@kazuki.m---strapi/utils' },
    })
    .forEach((path) => {
  ...
  root.find(j.ImportNamespaceSpecifier).forEach((specifierPath) => {
    if (specifierPath.parent.value.source.value === '@kazuki.m---strapi/utils') {
      for (const primitive of Object.keys(changes)) {
        const functions = Object.keys(changes[primitive]);
  ...
  root
    .find(j.VariableDeclarator, {
      init: {
        ...
        arguments: [{ value: '@kazuki.m---strapi/utils' }],
      },
    })
    .forEach((path) => {
